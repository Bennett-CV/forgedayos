...
export default function AddFoodModal({ open, onClose, onAdded, defaultMealType, date }) {
  ...
  async function handleAnalyze() {
    ...
    setPreview(null);
    try {
      // Prepend quantity to food input so the AI handles the conversion correctly
      const qty = quantity.trim();
      const combinedInput = qty ? `${qty} ${foodInput.trim()}` : foodInput.trim();
      const response = await base44.functions.invoke("analyzeFoodUSDA", { foodInput: combinedInput });
  ...
  async function handleSave() {
    ...
    try {
      ...
      const qty = quantity.trim() ? quantity.trim() : "1";
      await base44.entities.Meal.create({
        date: mealDate,
        meal_type: mealType,
        food_description: qty !== "1" ? `${qty} ${foodInput.trim()}` : foodInput,
        quantity: 1,
        calories: preview.calories || 0,
        protein_g: preview.protein_g || 0,
        carbs_g: preview.carbs_g || 0,
        fat_g: preview.fat_g || 0,
        fiber_g: preview.fiber_g || null,
        sugar_g: preview.sugar_g || null,
  ...
  const content = (
    <div className="space-y-4 px-4 pb-6">
      ...
      {/* Food input + quantity + analyze button */}
      <div>
        <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-1.5 block">Food</label>
        <div className="flex gap-2 mb-2">
          <Input
            ...
            className="bg-secondary/50 border-border w-24 shrink-0"
          />
          <Input
            placeholder="e.g. eggs, bacon strips, ground beef"
            value={foodInput}
        ...
        <p className="text-[10px] text-muted-foreground">Enter quantity + food name, then tap ✦ to analyze</p>
      </div>
      ...
      {preview && (
        <div className="rounded-xl border border-border bg-secondary/30 p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: "Calories", val: Math.round(preview.calories || 0), unit: "kcal", color: "text-chart-3" },
              { label: "Protein", val: Math.round(preview.protein_g || 0), unit: "g", color: "text-chart-1" },
              { label: "Carbs", val: Math.round(preview.carbs_g || 0), unit: "g", color: "text-chart-4" },
              { label: "Fat", val: Math.round(preview.fat_g || 0), unit: "g", color: "text-chart-2" },
              { label: "Fiber", val: preview.fiber_g ? Math.round(preview.fiber_g) : "—", unit: "g", color: "text-muted-foreground" },
              { label: "Sugar", val: preview.sugar_g ? Math.round(preview.sugar_g) : "—", unit: "g", color: "text-muted-foreground" },
